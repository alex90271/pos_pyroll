...
class QueryDB():
    ...
    def __init__(self, data=''):
        ...
        if data == datetime.datetime.today().strftime("%Y%m%d"):  # if the date is TODAY, use latest data
            ...
            # print('WARNING: Using data from the Data folder. This data should be considered unoffical and may change')
            self.data = 'Data'
        # if someone tries to use a future day, throw an error.
        elif data > datetime.datetime.today().strftime("%Y%m%d"):
            raise ValueError(
                'ERROR: Future data cannot be used. It does not exist')
        else:
    ...
    def dbf_to_list(self, dbf):
        ...
        try:
            dbf_list = DBF(path + self.data + dbf,
                           char_decode_errors='ignore', load=False)
        except:
            raise ValueError(path + self.data + dbf +
    ...
    def process_db(self, db_type):
        ...
        if db_type == 'employees':
            ...
            if ChipConfig().query('SETTINGS', 'debug'):
                ...
                d = pd.DataFrame(
                    a, columns=['ID', 'SHORTNAME']).sort_values(by='ID')
            return d.copy()
        elif db_type == 'labor':
            if self.data == 'Data':
            ...
            if ChipConfig().query('SETTINGS', 'debug'):
                ...
                df['HOURS'] = np.subtract(
                    df['HOURS'].values, df['OVERHRS'].values)
                return df.copy()
            ...
            b_df = pd.DataFrame(self.dbf_to_list('/GNDSLSUM.DBF'))
            try:
                b_df = b_df.loc[np.where(b_df['KEYVOLUME'] == 1)]
                b_df = b_df.loc[np.where(b_df['CATID'] == 10)]
                b_df.drop(columns=['REVID', 'CATID',
                          'KEYVOLUME'], axis=1, inplace=True)
            except:
                print(self.data + ' was empty')
                return pd.DataFrame([])
            # merge them on their shared columns and return the result
            df = a_df.merge(b_df, on=shared_cols, how='left')
            return df.copy()
...
if __name__ == '__main__':
    ...
    f = timeit.repeat("main()", "from __main__ import main",
                      number=1, repeat=r)
    print("completed with an average of " + str(np.round(np.mean(f), 6)) + " seconds over " +
