...
class MainGui():
    ...
    def launch(self):
        ...
        day_two = (date.today()-timedelta(days=7)).strftime('%Y%m%d')
        rpt_type = 'labor_main'
        select_jobs = []
        select_emps = []
        ...
        def open_popup_box():
            ...
            psl.pack(pady=5)
            print()
            date_dropdown = tk.OptionMenu(popup_window, "", *[(today - timedelta(days=i)) for i in range()])
            date_dropdown.pack()


            psl = tk.Label(popup_window, text="Primary Server")
            psl.pack(pady=5)
            primary_selection_listbox = tk.Listbox(popup_window,listvariable=tk.StringVar(l_frame, value=self.employee_df["NAME"].to_list()), selectmode="single", exportselection=False)
            primary_selection_listbox.pack()


            msl = tk.Label(popup_window, text="Secondary Servers")
            msl.pack(pady=5)
            multiple_selection_listbox = tk.Listbox(popup_window,listvariable=tk.StringVar(l_frame, value=self.employee_df["NAME"].to_list()), selectmode="multiple", exportselection=False)
        ...
        add_adjustment_button = tk.Button(self.root, text="Add Adjustment", command=open_popup_box)
        add_adjustment_button.pack()
        ...
        report_dropdown = tk.OptionMenu(
            l_frame, dropdown_val, *self.reports, command=on_report_changed)
        ...
        employee_listbox = tk.Listbox(
            l_frame, listvariable=tk.StringVar(l_frame, value=self.employee_df["NAME"].to_list()), selectmode='multiple')
        employee_listbox.pack()

        # Set the callback function
        def on_employee_changed(event):
            global select_emps
            select_emps = [self.employee_df.index[i]
        ...
        jobcode_listbox = tk.Listbox(
            l_frame, listvariable=tk.StringVar(l_frame, value=self.jobcode_df["SHORTNAME"].to_list()), selectmode='multiple')
        jobcode_listbox.pack()

        # Set the callback function
        def on_jobcode_changed(event):
            global select_jobs
            select_jobs = [self.jobcode_df.index[i]
